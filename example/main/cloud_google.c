...
{
    ESP_LOGI(CLOUD_TAG, "Initializing SNTP");
...
static void obtain_time(void)
{
    ...
    while (timeinfo.tm_year < (2016 - 1900)) {
        ESP_LOGI(CLOUD_TAG, "Waiting for system time to be set...");
        vTaskDelay(2000 / portTICK_PERIOD_MS);
        time(&now);
        localtime_r(&now, &timeinfo);
    }
    ESP_LOGI(CLOUD_TAG, "Time is set...");
}
    ...
    void *user_data)
{
    ...
    if (params != NULL && params->message.topic != NULL) {
        ESP_LOGI(CLOUD_TAG, "Subscription Topic: %s", params->message.topic);
        char *sub_message = (char *)malloc(params->message.temporary_payload_data_length + 1);
        if (sub_message == NULL) {
            ESP_LOGE(CLOUD_TAG, "Failed to allocate memory");
            return;
        }
                                 ...
                                 void *data, iotc_state_t state)
{
    ...
    case IOTC_CONNECTION_STATE_OPENED:
        ...
        ESP_LOGI(CLOUD_TAG, "subscribing to topic: \"%s\"", subscribe_topic_command);
        ...
        ESP_LOGI(CLOUD_TAG, "subscribing to topic: \"%s\"", subscribe_topic_config);
        ...
        delayed_publish_task = iotc_schedule_timed_task(in_context_handle,
                               publish_telemetry_event, publish_timer,
                               15, /*user_data=*/NULL);
        break;
    ...
    default:
        ESP_LOGE(CLOUD_TAG, "incorrect connection state value.");
        break;
    }
}
...
static void mqtt_task(void *pvParameters)
{
    ...
    * GCP IoT Core Service. */
    ...
    if (IOTC_STATE_OK != error_init) {
        ESP_LOGE(CLOUD_TAG, " iotc failed to initialize, error: %d", error_init);
        vTaskDelete(NULL);
    }
    ...
    if (IOTC_INVALID_CONTEXT_HANDLE >= iotc_context) {
        ESP_LOGE(CLOUD_TAG, " iotc failed to create context, error: %d", -iotc_context);
        vTaskDelete(NULL);
    }
    ...
    if (IOTC_STATE_OK != state) {
        ESP_LOGE(CLOUD_TAG, "iotc_create_iotcore_jwt returned with error: %ul", state);
        vTaskDelete(NULL);
    }
